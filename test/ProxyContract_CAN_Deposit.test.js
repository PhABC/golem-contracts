...
var GolemNetworkTokenBatching = artifacts.require('GolemNetworkTokenBatchingFixOne');
var GNTDeposit = artifacts.require('GNTDeposit');
var ProxyMigrateTokensAndDeposit = artifacts.require('ProxyMigrateTokensAndDepositFixOne');
...
contract('ProxyMigrateTokensAndDepositFixOne', function ([owner, anyone, Alice, Bob]) {
  ...
  describe('Testing Proxy contract trying to deposit GNT in GNTDeposit', function() {
    beforeEach(async function () {
      ...
      // Alice sends all their GNT to ProxyMigrateTokensAndDeposit gate
      await this.GNT.transfer(gate, amountToken, {from: Alice});

      // Alice sets approval to proxyMigrateTokensAndDeposit contract
      await this.GNTB.approve(this.proxyMigrateTokensAndDeposit.address, amountToken, {from: Alice});
    ...
    context('once all contracts are deployed and Alice has GNT tokens', function () {
      ...
      describe('When Alice calls migrateTokensAndDeposit()', function () {
        it('it should deposit Alice GNTB in GNTDeposit contract', async function () {
          var depositBalance = await this.deposit.balanceOf(Alice);
        ...
        context('If Alice tries to unlock the tokens on the  GNT Deposit contract', function () {
          beforeEach(async function (){
            ...
            await this.deposit.unlock({from: Alice});
          });

          it('it should set isLocked to false', async function () {
            var isLocked = await this.deposit.isLocked(Alice);
          ...
          context('But if Bob also calls migrateTokensAndDeposit()', function (){
            ...
            beforeEach(async function () { 
              ...
              // Bob sets allowance to proxy contract
              await this.GNTB.approve(this.proxyMigrateTokensAndDeposit.address, amountToken, {from: Bob});

              //Bob calls migrateTokensAndDeposit()
              await this.proxyMigrateTokensAndDeposit.migrateTokensAndDeposit(amountToken, {from: Bob});
            });

            it('Alice tokens will NOT be locked again (they use same proxy contract)', async function () {
                var isLocked = await this.deposit.isLocked(Alice);
                isLocked.should.be.equal(false);
            });
          });
