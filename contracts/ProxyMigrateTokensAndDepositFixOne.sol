...
import "./GolemNetworkTokenBatchingFixOne.sol";
...
contract ProxyMigrateTokensAndDepositFixOne is StandardToken, BurnableToken {
  ...
  GolemNetworkTokenBatchingFixOne public GNTB; // GNTB contract
  ...
  //Constructor
  function ProxyMigrateTokensAndDepositFixOne(
      GNTDeposit _depositContract,
      ERC20Basic _GNT,
      GolemNetworkTokenBatchingFixOne _GNTB)
      public
  { 
    //Setting contracts
  ...
  function migrateTokensAndDeposit(uint256 _value) public {
    ...
    //Transfer GNTB to msg.sender
    GNTB.transfer(msg.sender, _value);

    //Deposit GNTB to GNTDeposit contract
    GNTB.transferFromAndCall(msg.sender, depositContract, _value, '0x');
